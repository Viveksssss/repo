...
int main(int argc,char*argv[]){
    ...
    while(1){
        ...
        for(int i = 0;i<n;i++){
            ...
            //2
            int len = read(sockfd,buf,2); // 
            // 水平触发（LT）模式 的缺点之一是 可能会产生较多的重复通知，增加 CPU 开销。这句话的意思是，在 LT 模式下，只要文件描述符的状态满足条件（例如，有数据可读或可写），epoll_wait 就会持续通知应用程序，直到状态不再满足条件。这种机制可能会导致多次重复通知，从而增加 CPU 的开销
            if(len<=0){
                close(sockfd);
